...
const io = new Server(httpServer, {
  cors: {
    origin: [
      "http://localhost:4200",
      "http://localhost:5173", 
      "https://aptitude-game-site-h2je.onrender.com"
    ],
    methods: ["GET", "POST"],
    credentials: true,
  },
    ...
    socket.on("createRoom", async({ username, settings }, callback) => {
        try {
            ...
            if (settings.domain === "Mixed") {
                ...
                // Use aggregation with $sample for random selection instead of limit
                questions = await Question.aggregate([
                    { $match: { domain: settings.domain } },
...
app.get("/api/random-question", async (req, res) => {
    try {
        const { domain } = req.query; // domain from frontend, e.g., "Math"
        let pipeline = [];
        ...
        if (question.length === 0) {
            return res.status(404).json({ message: "No question found for this domain" });
        }

        res.json(question[0]); // return single question object
    } catch (err) {
